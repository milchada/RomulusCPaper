...
def maps(maptype, select, stime, snapnum, width='100 kpc', retimg = True):
	...
	if maptype == 'temp':
		img = image(select.g, qty='temp',width=width, qtytitle='T_kev', units='keV', log='False',
			av_z='mass', title='%0.2f' % stime, cmap=cm.RdBu_r, vmin = .1, vmax = 5, filename='cold_temp_%d' % snapnum)
...
def make_map(snapnum, maptype='rho',width='500 kpc'):
